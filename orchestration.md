...
| Multi-repo changes — a distinct **code-root/commit-root** from the plan-root, or a change whose deliverable is a **new standalone repo** | Not deferred — **committed constraint** (ADR-0004; §1, §9): the run worktree is a worktree *of the plan's repo* and is simultaneously the plan-, code-, and commit-root; the single root is the trust story (one mount = the agents' whole writable world, one commit root, out-of-paths work fails loud via `empty_paths`). Split such changes by hand. [#24](https://github.com/kentra-io/agent-orchestration/issues/24) closed as by-design. |
